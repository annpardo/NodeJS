...
[ "$REALITY_PORT" != "" ] && add_link "vless://${'${'}UUID}@${'${'}IP}:${'${'}REALITY_PORT}?encryption=none&flow=xtls-rprx-vision&security=reality&sni=${'${'}REALITY_SNI}&fp=firefox&pbk=${'${'}public_key}&type=tcp#Reality-${'${'}ISP}"
add_argo_vless_link() {
  local server="$1"
  local port="$2"
...
if [ "$CF_MODE" = "cdn" ]; then
  ...
  add_argo_vless_link "$ARGO_HOST" "443"
fi
...
# ================== 定时删除敏感文件 ==================
INITIAL_SINGBOX_PATH="${'${'}FILE_MAP[sing-box]}"
INITIAL_ARGO_PATH=""
[ "$CF_MODE" = "argo" ] && INITIAL_ARGO_PATH="${'${'}FILE_MAP[argo]}"
{
  sleep 20
  rm -f "$ARGO_LOG" "${'${'}FILE_PATH}/config.json" "$INITIAL_SINGBOX_PATH" "$INITIAL_ARGO_PATH" 2>/dev/null || true
...
[ "$TUIC_PORT" != "" ] && [ "$TUIC_PORT" != "0" ] && add_inbound "$(cat <<EOF
{"type":"tuic","listen":"::","listen_port":$TUIC_PORT,"users":[{"uuid":"$UUID","password":"admin"}],"congestion_control":"bbr","tls":{"enabled":true,"alpn":["h3"],"certificate_path":"$FILE_PATH/cert.pem","key_path":"$FILE_PATH/private.key"}}
EOF
  )"
[ "$HY2_PORT" != "" ] && [ "$HY2_PORT" != "0" ] && add_inbound "$(cat <<EOF
{"type":"hysteria2","listen":"::","listen_port":$HY2_PORT,"users":[{"password":"$UUID"}],"masquerade":"https://bing.com","tls":{"enabled":true,"alpn":["h3"],"certificate_path":"$FILE_PATH/cert.pem","key_path":"$FILE_PATH/private.key"}}
EOF
  )"
[ "$REALITY_PORT" != "" ] && add_inbound "$(cat <<EOF
{"type":"vless","listen":"::","listen_port":$REALITY_PORT,"users":[{"uuid":"$UUID","flow":"xtls-rprx-vision"}],"tls":{"enabled":true,"server_name":"$REALITY_SNI","reality":{"enabled":true,"handshake":{"server":"$REALITY_SNI","server_port":443},"private_key":"$private_key","short_id":[""]}}}
EOF
  )"
if [ "$CF_MODE" = "argo" ]; then
  add_inbound "$(cat <<EOF
{"type":"vless","listen":"127.0.0.1","listen_port":$ARGO_PORT,"users":[{"uuid":"$UUID"}],"transport":{"type":"ws","path":"/vless-ws","max_early_data":2560,"early_data_header_name":"Sec-WebSocket-Protocol"}}
EOF
  )"
elif [ "$CF_MODE" = "cdn" ]; then
  add_inbound "$(cat <<EOF
{"type":"vless","listen":"::","listen_port":$CF_ORIGIN_PORT,"users":[{"uuid":"$UUID"}],"tls":{"enabled":true,"certificate_path":"$FILE_PATH/cert.pem","key_path":"$FILE_PATH/private.key"},"transport":{"type":"ws","path":"/vless-ws","max_early_data":2560,"early_data_header_name":"Sec-WebSocket-Protocol"}}
EOF
  )"
fi
write_singbox_config() {
...
OLD_SINGBOX_PID=$(cat "$FILE_PATH/sx.pid" 2>/dev/null || true)
if [ -n "$OLD_SINGBOX_PID" ]; then kill "$OLD_SINGBOX_PID" 2>/dev/null || true; fi
sleep 3
write_singbox_config
declare -A FILE_MAP
download_component "sing-box" "sbx-1.13.13"
nohup "${'${'}FILE_MAP[sing-box]}" run -c "$FILE_PATH/config.json" > /dev/null 2>&1 &
SINGBOX_PID=$!
printf "%s\n" "$SINGBOX_PID" > "$FILE_PATH/sx.pid"
RESTARTED_SINGBOX_PATH="${'${'}FILE_MAP[sing-box]}"
{ sleep 20; rm -f "$RESTARTED_SINGBOX_PATH" "$FILE_PATH/config.json" 2>/dev/null || true; echo "[清理] 已删除 config.json、核心程序文件"; } &
...
child.on('close', (code) => {
  ...
  if (!schedulerStarted) {
    ...
    if (restartTime && getNextBeijingRestart(restartTime) !== null) console.log(`[定时重启:Sing-box] 已启动（北京时间 ${restartTime}）`);
  }
});
...
function runCoreRestart() {
  ...
  restartRunning = true;
  console.log(`[定时重启:Sing-box] 到达 ${restartTime} → 重启 Sing-box`);
  ...
  restartChild.on('close', (code) => {
    if (code !== 0) console.error(`[定时重启:Sing-box] 重启失败，退出码=${code}`);
    finishRestart();
  });
}
