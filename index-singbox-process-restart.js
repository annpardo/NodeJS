...
const RESTART_TIME   = process.env.RESTART_TIME   || '';         // 北京时间定时重启sing-box，格式HH:MM
// ==============================================================
...
function createSingBoxProcess(initialLibraryPath) {
  let child = null;
  let active = false;
  let stopping = false;
  let operation = Promise.resolve();
  ...
  function start(libraryPath = currentLibraryPath) {
    ...
    return new Promise((resolve, reject) => {
      const worker = spawn(process.execPath, [__filename, '--singbox-worker', libraryPath], {
        cwd: __dirname,
        env: process.env,
        stdio: 'inherit'
      });
      ...
      worker.once('spawn', () => {
        active = true;
        settled = true;
        log(`[SING-BOX] 子进程已启动 PID=${worker.pid}`);
        resolve();
      });
      worker.once('error', error => {
        ...
        log(`[SING-BOX] 子进程启动失败: ${error.message}`);
      });
      worker.once('exit', (code, signal) => {
        active = false;
        if (child === worker) child = null;
        if (!stopping && (code !== 0 || signal)) {
          log(`[SING-BOX] 子进程退出，退出码=${code === null ? 'null' : code}${signal ? `，信号=${signal}` : ''}`);
        }
      });
    });
  }
...
async function runSingBoxWorker(libraryPath) {
  const lib = koffi.load(libraryPath);
  const startFn = lib.func('int StartSingBox(str)');
  const stopFn = lib.func('int StopSingBox()');
  ...
  const stopWorker = () => new Promise(resolve => {
    ...
    try {
      stopFn.async((err) => {
        if (err) log(`[SING-BOX] 停止失败: ${err.message}`);
        resolve();
      });
    } catch (error) {
      log(`[SING-BOX] 停止失败: ${error.message}`);
      resolve();
    }
  });
  ...
  try {
    startFn.async(JSON.stringify({ config: singBoxConfigPath, workingDir: '.', disableColor: true }), (err, code) => {
      if (stoppingWorker) return;
      if (err) log(`[SING-BOX] 运行失败: ${err.message}`);
      else if (code !== 0) log(`[SING-BOX] 已退出，退出码=${code}`);
      clearInterval(keepAlive);
...
function createRestartScheduler(singBoxService, writeConfig, downloadSingBox) {
  let timer = null;
  let stopped = false;
  ...
  function schedule() {
    ...
    timer = setTimeout(async () => {
      try {
        ...
        log(`[定时重启:Sing-box] 到达北京时间 ${restartTime}，正在重启`);
        if (HY2_PORT || TUIC_PORT || cfMode === 'cdn') ensureTlsCertificates(certPath, tlsKeyPath);
        writeConfig();
        await singBoxService.stop();
        const freshLibraryPath = await downloadSingBox();
        await singBoxService.start(freshLibraryPath);
        scheduleRuntimeCleanup();
        scheduleConsoleCleanup();
        log('[定时重启:Sing-box] 重启完成');
      } catch (error) {
        log(`[定时重启:Sing-box] 重启失败: ${error.message}`);
      } finally {
        schedule();
      }
  ...
  if (restartTime) {
    schedule();
    if (timer) log(`[定时重启:Sing-box] 已启动（北京时间 ${restartTime}）`);
  } else {
...
async function startServer() {
  ...
  // sing-box
  const singBoxService = createSingBoxProcess(singBoxLib);
  ...
  stopRestartScheduler = createRestartScheduler(
    singBoxService,
    writeSingBoxConfig,
    () => downloadLibrary(`${baseUrl}/sbx.so`, 'sbx.so', true)
  );
...
if (process.argv[2] === '--singbox-worker') {
  runSingBoxWorker(process.argv[3]).catch(error => {
    console.error('[SING-BOX] 子进程启动失败', error.stack || error.message);
    process.exit(1);
  });
} else {
