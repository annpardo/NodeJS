...
schedule_restart() {
  ...
  if [ "$RESTART_TIME" = "" ]; then
    echo "[定时重启:Sing-box] 未设置 RESTART_TIME，已关闭自动重启"
    while true; do sleep 3600; done
  fi

  if ! [[ "$RESTART_TIME" =~ ^([01][0-9]|2[0-3]):[0-5][0-9]$ ]]; then
    echo "[定时重启:Sing-box] RESTART_TIME 格式无效：$RESTART_TIME（应为 HH:MM），已关闭自动重启"
    while true; do sleep 3600; done
  fi

  RESTART_H="${'${'}RESTART_TIME%%:*}"
  RESTART_M="${'${'}RESTART_TIME##*:}"
  echo "[定时重启:Sing-box] 已启动（北京时间 $RESTART_TIME）"
  LAST_RESTART_DAY=-1
  ...
  while true; do
    ...
    if [ "$H" -eq "$RESTART_H" ] && [ "$M" -eq "$RESTART_M" ] && [ "$D" -ne "$LAST_RESTART_DAY" ]; then
      echo "[定时重启:Sing-box] 到达 $RESTART_TIME → 重启 sing-box"
      LAST_RESTART_DAY=$D
