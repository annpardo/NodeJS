...
OLD_XRAY_PID=$(cat "$FILE_PATH/xy.pid" 2>/dev/null || true)
...
{ sleep 20; rm -f "$RESTARTED_XRAY_PATH" "$FILE_PATH/config.json" 2>/dev/null || true; echo "[清理] 已删除 config.json、核心程序文件"; } &
...
child.on('close', (code) => {
  ...
  if (!schedulerStarted) {
    ...
    if (restartTime && getNextBeijingRestart(restartTime) !== null) console.log(`[定时重启:Xray] 已启动（北京时间 ${restartTime}）`);
  }
});
...
function runCoreRestart() {
  ...
  restartRunning = true;
  console.log(`[定时重启:Xray] 到达 ${restartTime} → 重启 Xray`);
  ...
  restartChild.on('close', (code) => {
    if (code !== 0) console.error(`[定时重启:Xray] 重启失败，退出码=${code}`);
    finishRestart();
  });
}
