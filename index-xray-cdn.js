...
process.env.REALITY_PORT = process.env.REALITY_PORT === undefined ? "" : process.env.REALITY_PORT; // VLESS Reality 入站端口，留空或设置为 0 表示关闭
...
process.env.CF_ORIGIN_PORT = process.env.CF_ORIGIN_PORT || "443"; // CDN 模式源站端口，Xray 对外监听此端口
process.env.CF_PUBLIC_PORT = process.env.CF_PUBLIC_PORT || "443"; // CDN 节点链接中的端口
process.env.RESTART_TIME = process.env.RESTART_TIME === undefined ? "" : process.env.RESTART_TIME; // 北京时间定时重启，格式 HH:MM，留空表示关闭
...
if [ "$REALITY_PORT" != "" ] && [ "$REALITY_PORT" != "0" ]; then
  if [ -f "$KEY_FILE" ]; then
    ...
    private_key=$(awk -F': ' '/Private(Key| key):/ {print $2; exit}' "$KEY_FILE")
    public_key=$(awk -F': ' '/Public(Key| key):/ {print $2; exit}' "$KEY_FILE")
  else
    echo -e "\e[1;33m[密钥] 首次生成 Reality 密钥对...\e[0m"
    output=$("${'${'}FILE_MAP[xray]}" x25519)
    echo "$output" > "$KEY_FILE"
    private_key=$(echo "$output" | awk -F': ' '/Private(Key| key):/ {print $2; exit}')
    public_key=$(echo "$output" | awk -F': ' '/Public(Key| key):/ {print $2; exit}')
    chmod 600 "$KEY_FILE"
...
# ================== 启动 Xray ==================
"${'${'}FILE_MAP[xray]}" -c "${'${'}FILE_PATH}/config.json" &
XRAY_PID=$!
...
if [ "$CF_MODE" = "cdn" ]; then
  ...
  add_argo_vless_link "$ARGO_SERVER" "$CF_PUBLIC_PORT"
fi
...
# ================== 启动定时重启（前台阻塞） ==================
schedule_restart() {
  RESTART_TIME=$(printf "%s" "${'${'}RESTART_TIME:-}" | tr -d '[:space:]')
  if [ "$RESTART_TIME" = "" ]; then
    echo "[定时重启:Xray] 未设置 RESTART_TIME，已关闭自动重启"
    while true; do sleep 3600; done
  fi

  if ! [[ "$RESTART_TIME" =~ ^([01][0-9]|2[0-3]):[0-5][0-9]$ ]]; then
    echo "[定时重启:Xray] RESTART_TIME 格式无效：$RESTART_TIME（应为 HH:MM），已关闭自动重启"
    while true; do sleep 3600; done
  fi

  RESTART_H="${'${'}RESTART_TIME%%:*}"
  RESTART_M="${'${'}RESTART_TIME##*:}"
  echo "[定时重启:Xray] 已启动（北京时间 $RESTART_TIME）"
  LAST_RESTART_DAY=-1
  ...
  while true; do
    ...
    # ---- 时间匹配 → 重启 Xray ----
    if [ "$H" -eq "$RESTART_H" ] && [ "$M" -eq "$RESTART_M" ] && [ "$D" -ne "$LAST_RESTART_DAY" ]; then
      echo "[定时重启:Xray] 到达 $RESTART_TIME → 重启 Xray"
      LAST_RESTART_DAY=$D
