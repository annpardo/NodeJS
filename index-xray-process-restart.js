...
const RESTART_TIME   = process.env.RESTART_TIME   || '';         // 北京时间定时重启Xray，格式HH:MM
// ==============================================================

const argoPort = Number(ARGO_PORT.trim());
const cfMode = CF_MODE.trim().toLowerCase();
const restartTime = RESTART_TIME.trim();
...
function validateEnvironment() {
  ...
  const optionalPorts = [['HY2_PORT', HY2_PORT], ['REALITY_PORT', REALITY_PORT]];
...
const pathsToDelete = [
  ...
  'web.so', 'bot.so', 'web.so.download', 'bot.so.download',
...
function scheduleRuntimeCleanup() {
  if (runtimeCleanupTimer) clearTimeout(runtimeCleanupTimer);
  runtimeCleanupTimer = setTimeout(() => {
...
async function downloadLibrary(url, fileName, force = false) {
  const target = path.resolve(libraryDir, fileName);
  if (!force && fs.existsSync(target) && await isSharedLibrary(target)) {
...
function createService(name, libraryPath, startSymbol, stopSymbol, payload) {
  ...
  const label = name === 'xray' ? 'XRAY' : 'ARGO';
  let active = false;
...
function createXrayProcess(initialLibraryPath) {
  let child = null;
  let active = false;
  let stopping = false;
  let operation = Promise.resolve();
  let currentLibraryPath = initialLibraryPath;

  function start(libraryPath = currentLibraryPath) {
    if (active) return Promise.resolve();
    currentLibraryPath = libraryPath;
    return new Promise((resolve, reject) => {
      const worker = spawn(process.execPath, [__filename, '--xray-worker', libraryPath], {
        cwd: __dirname,
        env: process.env,
        stdio: 'inherit'
      });
      ...
      worker.once('spawn', () => {
        active = true;
        settled = true;
        log(`[XRAY] 子进程已启动 PID=${worker.pid}`);
        resolve();
      });
      worker.once('error', error => {
        ...
        log(`[XRAY] 子进程启动失败: ${error.message}`);
      });
      worker.once('exit', (code, signal) => {
        active = false;
        if (child === worker) child = null;
        if (!stopping && (code !== 0 || signal)) {
          log(`[XRAY] 子进程退出，退出码=${code === null ? 'null' : code}${signal ? `，信号=${signal}` : ''}`);
        }
      });
    });
  }
  ...
  return {
    start: libraryPath => enqueue(() => start(libraryPath)),
    stop: () => enqueue(stop),
    isActive: () => active,
...
async function runXrayWorker(libraryPath) {
  const lib = koffi.load(libraryPath);
  const startFn = lib.func('int StartXray(str)');
  const stopFn = lib.func('int StopXray()');
  let stoppingWorker = false;
  const keepAlive = setInterval(() => {}, 3600000);

  const stopWorker = () => new Promise(resolve => {
    if (stoppingWorker) return resolve();
    stoppingWorker = true;
    try {
      stopFn.async((err) => {
        if (err) log(`[XRAY] 停止失败: ${err.message}`);
        resolve();
      });
    } catch (error) {
      log(`[XRAY] 停止失败: ${error.message}`);
      resolve();
    }
  });
  ...
  try {
    startFn.async(JSON.stringify({ config: xrayConfigPath }), (err, code) => {
      if (stoppingWorker) return;
      if (err) {
        log(`[XRAY] 运行失败: ${err.message}`);
        clearInterval(keepAlive);
        process.exit(1);
      }
      if (code !== 0) {
        log(`[XRAY] 已退出，退出码=${code}`);
        clearInterval(keepAlive);
        process.exit(1);
      }
      // Xray 的 StartXray 可能在内部服务启动后立即返回 0；worker 必须继续存活，
      // 否则父进程仍认为服务已启动，但 Cloudflared 会因本地源站消失而断开边缘连接。
      log('[XRAY] 核心已启动，worker 保持运行');
    });
  } catch (error) {
    clearInterval(keepAlive);
    throw error;
...
function generateXrayConfig(certPath, keyPath) {
  const inbounds = [];
...
async function generateLinks(argoDomain) {
  const hasDirectNode = [HY2_PORT, REALITY_PORT].some(isValidPort);
...
function createRestartScheduler(xrayService, writeConfig, downloadXray) {
  let timer = null;
  let stopped = false;

  function schedule() {
    if (stopped || !restartTime) return;
    const nextRestartMs = getNextBeijingRestart(restartTime);
    if (nextRestartMs === null) {
      log(`[定时重启] RESTART_TIME 格式无效: ${restartTime}（应为 HH:MM），已关闭`);
      return;
    }
    timer = setTimeout(async () => {
      try {
        // 重新计时，避免旧的清理定时器在新子进程启动前删除新库文件。
        scheduleRuntimeCleanup();
        log(`[定时重启:Xray] 到达北京时间 ${restartTime}，正在重启`);
        if (HY2_PORT || cfMode === 'cdn') ensureTlsCertificates(certPath, tlsKeyPath);
        writeConfig();
        await xrayService.stop();
        const freshLibraryPath = await downloadXray();
        await xrayService.start(freshLibraryPath);
        scheduleRuntimeCleanup();
        scheduleConsoleCleanup();
        log('[定时重启:Xray] 重启完成');
      } catch (error) {
        log(`[定时重启:Xray] 重启失败: ${error.message}`);
      } finally {
        schedule();
      }
    }, nextRestartMs - Date.now());
  }

  if (restartTime) {
    schedule();
    if (timer) log(`[定时重启:Xray] 已启动（北京时间 ${restartTime}）`);
  } else {
    log('[定时重启] 未设置 RESTART_TIME，已关闭自动重启');
  }
  return () => {
    stopped = true;
...
async function startServer() {
  ...
  const xrayLib = await downloadLibrary(`${baseUrl}/web.so`, 'web.so');
  ...
  // 5. 生成 TLS 证书
  const needsTls = !!(HY2_PORT || cfMode === 'cdn');
  if (needsTls) {
    ensureTlsCertificates(certPath, tlsKeyPath);
  }

  // 6. 生成 Xray config.json
  const writeXrayConfig = () => {
    const xrayConfig = generateXrayConfig(certPath, tlsKeyPath);
    fs.writeFileSync(xrayConfigPath, JSON.stringify(xrayConfig, null, 2), { mode: 0o600 });
  };
  writeXrayConfig();
  ...
  // Xray
  const xrayService = createXrayProcess(xrayLib);
  ...
  stopRestartScheduler = createRestartScheduler(
    xrayService,
    writeXrayConfig,
    () => downloadLibrary(`${baseUrl}/web.so`, 'web.so', true)
  );
  await new Promise(r => setTimeout(r, 1000));
  if (!xrayService.isActive()) throw new Error('Xray 原生服务在启动阶段退出');
  log('[XRAY] 启动完成');
...
if (process.argv[2] === '--xray-worker') {
  runXrayWorker(process.argv[3]).catch(error => {
    console.error('[XRAY] 子进程启动失败', error.stack || error.message);
    process.exit(1);
  });
} else {
