...
export ARGO_ENABLE=${ARGO_ENABLE:-"true"}
...
if [[ "$ARCH" == "arm"* ]] || [[ "$ARCH" == "aarch64" ]]; then
  BASE_URL="https://arm64.jcharise.nyc.mn"
elif [[ "$ARCH" == "amd64"* ]] || [[ "$ARCH" == "x86_64" ]]; then
  BASE_URL="https://amd64.jcharise.nyc.mn"
elif [[ "$ARCH" == "s390x" ]]; then
  BASE_URL="https://s390x.jcharise.nyc.mn"
else
  echo "不支持的架构: $ARCH"
  exit 1
fi
...
for entry in "${FILE_INFOS[@]}"; do
  ...
  if [ -f "$BIN_PATH_FILE" ]; then
    SAVED_LINE=$(grep "^${NAME}=" "$BIN_PATH_FILE" | tail -n 1)
    SAVED_URL=$(echo "$SAVED_LINE" | cut -d '=' -f2)
    SAVED_NAME=$(echo "$SAVED_LINE" | cut -d '=' -f3-)
    if [ -x "$SAVED_NAME" ]; then
      if [ "$SAVED_URL" = "$DOWNLOAD_URL" ]; then
        FILE_MAP[$NAME]="$SAVED_NAME"
        echo -e "\e[1;33m复用 $NAME: $SAVED_NAME\e[0m"
        continue
      else
        echo -e "\e[1;33m$NAME 下载源已变更，重新下载...\e[0m"
      fi
    fi
  fi

  NEW_NAME="${FILE_PATH}/$(head /dev/urandom | tr -dc a-z0-9 | head -c6)"
  download_file "$DOWNLOAD_URL" "$NEW_NAME"
  chmod +x "$NEW_NAME"
  grep -v "^${NAME}=" "$BIN_PATH_FILE" 2>/dev/null > "${BIN_PATH_FILE}.tmp" || true
  echo "${NAME}=${DOWNLOAD_URL}=${NEW_NAME}" >> "${BIN_PATH_FILE}.tmp"
  mv "${BIN_PATH_FILE}.tmp" "$BIN_PATH_FILE"
...
[ "$ARGO_ENABLE" = "true" ] && add_inbound "$(cat <<EOF
{
  "type": "vless",
  "listen": "127.0.0.1",
...
[ "$REALITY_PORT" != "" ] && [ "$REALITY_PORT" != "0" ] && add_link "vless://${UUID}@${IP}:${REALITY_PORT}?encryption=none&flow=xtls-rprx-vision&security=reality&sni=www.nazhumi.com&fp=firefox&pbk=${public_key}&type=tcp#Reality-${ISP}"
add_argo_vless_link() {
  local server="$1"
  local port="$2"
...
if [ "$ARGO_HOST" != "" ]; then
  ...
  if [ "$CF_PREFERRED_DOMAIN" != "" ]; then
    ARGO_SERVER="$CF_PREFERRED_DOMAIN"
  fi
  add_argo_vless_link "$ARGO_SERVER" "$CF_PREFERRED_PORT"
fi

printf "%s" "$LINKS" | base64 | tr -d '\n' > "${FILE_PATH}/sub.txt"
